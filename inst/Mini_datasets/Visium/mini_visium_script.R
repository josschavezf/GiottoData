...
# 2 process ####
# ------------ #
mini_visium <- normalizeGiotto(gobject = mini_visium,
                               scalefactor = 6000,
                               verbose = TRUE)
...
mini_visium <- runtSNE(mini_visium,
                       dimensions_to_use = 1:10)
...
mini_visium <- createSpatialNetwork(
    gobject = mini_visium,
    method = "kNN", k = 10,
...
# 7.3 identify potential spatial co-expression
...
cluster_genes <- getBalancedSpatCoexpressionFeats(spat_cor_netw_DT,
                                                  maximum = 30)
mini_visium <- createMetafeats(mini_visium,
...
mini_visium <- createNearestNetwork(
    gobject = mini_visium,
    dim_reduction_name = "custom_pca",
    dimensions_to_use = 1:20,
    k = 5,
...
# remake
mini_visium_remake <- createGiottoObject(
    expression = list(
        "cell" =
            list(
                "rna" =
...
mini_visium_remake <- set_cell_metadata(
    gobject = mini_visium_remake,
    metadata = cell_metadata,
    spat_unit = "cell", feat_type = "rna"
)

mini_visium_remake <- set_feature_metadata(
    gobject = mini_visium_remake,
    metadata = feat_metadata,
    spat_unit = "cell", feat_type = "rna"
)

pca_obj <- create_dim_obj(
    name = "pca", reduction = "cells", reduction_method = "pca",
    coordinates = pca_dim
)
umap_obj <- create_dim_obj(
    name = "umap", reduction = "cells", reduction_method = "umap",
    coordinates = umap_dim
)
tsne_obj <- create_dim_obj(
    name = "tsne", reduction = "cells", reduction_method = "tsne",
